...
export default function JobCardDetailScreen({ route, navigation }: Props) {
  ...
  const doStatusUpdate = async (newStatus: string) => {
    setUpdating(true);
    try {
      await updateJobCard(id, { status: newStatus as JobCard['status'] });
      Toast.show({ type: 'success', text1: 'Status updated' });
      setJobCard(prev => prev ? { ...prev, status: newStatus as JobCard['status'] } : prev);
    } catch (error) {
      Toast.show({ type: 'error', text1: getErrorMessage(error, 'Failed to update status') });
    } finally {
      setUpdating(false);
    }
  };
  ...
  const invoiceId = jobCard?.invoice
    ? (typeof jobCard.invoice === 'object' ? jobCard.invoice._id : jobCard.invoice)
    : null;
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          <StatusStepper
            currentStatus={jobCard?.status || 'new'}
        ...
        <View style={styles.section}>
          ...
          {hasInvoice ? (
            <TouchableOpacity
              ...
              onPress={() => invoiceId && navigation.navigate('InvoiceViewer', { invoiceId })}
              activeOpacity={0.85}
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.invoiceNumber}>
                  {jobCard?.invoice && typeof jobCard.invoice === 'object' ? jobCard.invoice.invoiceNumber : 'View Invoice'}
                </Text>
