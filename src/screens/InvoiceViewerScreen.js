...
export default function InvoiceViewerScreen({ route, navigation }) {
        ...
        { text: 'Keep Invoice', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Toast.show({ type: 'error', text1: e?.response?.data?.message || 'Failed to cancel invoice' });
            }
          }
        }
      ]
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent}>
        ...
        <View style={s.card}>
          ...
          {invoice.jobCard?.jobCardNumber && (
            <View style={s.metaRow}>
              ...
              <Text style={s.metaValue}>{invoice.jobCard.jobCardNumber}</Text>
            </View>
          )}
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          <View style={[s.card, { flex: 1 }]}>
            ...
            <Text style={s.cardTitle}>{invoice.customer?.name || '—'}</Text>
            {invoice.customer?.phone && (
              <Text style={s.cardSub}>📞 {invoice.customer.phone}</Text>
            )}
            {invoice.customer?.email && (
              <Text style={s.cardSub}>✉️ {invoice.customer.email}</Text>
            )}
          </View>
          <View style={[s.card, { flex: 1 }]}>
            <Text style={s.cardLabel}>VEHICLE</Text>
            <Text style={s.cardTitle}>{invoice.vehicle?.licensePlate || '—'}</Text>
