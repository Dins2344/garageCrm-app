...
const FUEL_COLOR = { petrol: '#ef4444', diesel: '#3b82f6', electric: '#10b981', hybrid: '#8b5cf6', cng: '#f59e0b' };
const BLANK = { licensePlate: '', make: '', model: '', year: '', color: '', fuelType: 'petrol', customer: '' };
...
// ─── Add / Edit Modal ─────────────────────────────────────────────────────────
function VehicleModal({ visible, onClose, onSave, editing, customers }) {
  ...
  useEffect(() => {
    if (visible) {
      setForm(editing ? {
        ...
        customer: editing.customer?._id || editing.customer || '',
      } : BLANK);
    }
  }, [visible, editing]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, licensePlate: form.licensePlate.toUpperCase() };
      if (payload.year) payload.year = parseInt(payload.year);
      else delete payload.year;
      await onSave(payload);
      onClose();
    } catch (e) {
      Toast.show({ type: 'error', text1: e?.response?.data?.message || 'Failed to save vehicle' });
    } finally { setSaving(false); }
  };

  const F = ({ label, value, onChange, placeholder, keyboard, cap }) => (
    <View style={s.field}>
...
export default function VehiclesScreen({ navigation }) {
  ...
  const fetchVehicles = useCallback(async (currentPage = 1, refresh = false) => {
    ...
    try {
      ...
      setVehicles(currentPage === 1 ? data : prev => [...prev, ...data]);
  ...
  const renderItem = ({ item: v }) => {
    const fuel = v.fuelType?.toLowerCase() || 'petrol';
      ...
      <TouchableOpacity style={s.card} activeOpacity={0.8}
        ...
        <View style={s.cardFooter}>
          <View style={s.metaRow}>
            ...
            <Text style={s.metaText}>{v.customer?.name || 'No owner'}</Text>
          </View>
  ...
  return (
    <View style={s.container}>
      ...
      {loading ? (
        ...
        <FlatList data={vehicles} keyExtractor={i => i._id} renderItem={renderItem}
...
const s = StyleSheet.create({
  ...
  vehicleIcon: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#eff2ff', justifyContent: 'center', alignItems: 'center' },
  plate: { fontSize: 17, fontWeight: 'bold', color: '#111827', letterSpacing: 0.5 },
  makeModel: { fontSize: 12, color: '#6b7280', marginTop: 2 },
  fuelBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 100 },
  ...
  // Customer picker
  searchRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f9fafb', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, paddingHorizontal: 10, height: 42, marginBottom: 8 },
  searchInput: { flex: 1, fontSize: 14, color: '#1f2937' },
  customerList: { maxHeight: 160, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, marginBottom: 8 },
  ...
  selectedChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#f0fdf4', borderWidth: 1, borderColor: '#bbf7d0', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
