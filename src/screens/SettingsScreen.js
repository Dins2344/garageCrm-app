...
function Field({ label, value, onChangeText, placeholder, keyboardType, autoCapitalize, editable = true, secureTextEntry }) {
...
export default function SettingsScreen({ navigation }) {
  ...
  const handleSaveGarage = async () => {
    ...
    try {
      const { data } = await updateGarage({
        ...
        settings: { taxRate: Number(garageTax) || 18, laborRatePerHour: Number(garageLabor) || 500 },
      });
      setGarage(data);
      setEditingGarage(false);
      Toast.show({ type: 'success', text1: 'Garage info updated!' });
    } catch (e) {
      Toast.show({ type: 'error', text1: e?.response?.data?.message || 'Failed to update garage' });
  ...
  const ROLE_LABEL = { owner: 'Owner', admin: 'Admin', service_advisor: 'Service Advisor', mechanic: 'Mechanic', receptionist: 'Receptionist' };
        ...
        <View style={styles.profileHero}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.heroRoleBadge}>
              <Text style={styles.heroRoleText}>{ROLE_LABEL[user?.role] || user?.role}</Text>
            </View>
          </View>
